...
const spaceships: Spaceship[] = [];
 ...
 function createSpaceship(name: string, pilot: string, crewLimit: number): void {
...
function addCrewMember(shipName: string, member: string): void {
...
function sendToMission(shipName: string): void {
  const ship = spaceships.find((s) => s.name === shipName);
  if (!ship) {
    console.log(`Nave "${shipName}" não encontrada.`);
    return;
  ...
  if (ship.crew.length < minCrew) {
    console.log(
      `A nave "${shipName}" precisa de pelo menos ${minCrew} tripulante(s) para partir.`
    );
    return;
  }
  ship.inMission = true;
